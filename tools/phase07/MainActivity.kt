...
class MainActivity : FlutterActivity() {
    companion object {
        ...
        private const val SHARE_CHANNEL = "school_schedule/share"
    ...
    private fun shareText(call: MethodCall, result: MethodChannel.Result) {
        try {
            ...
            if (text.isEmpty()) {
                result.error("share_empty", "Share text is empty.", null)
                return
            }
            ...
            startActivity(
                Intent.createChooser(intent, "مشاركة تطبيق التوقيت المدرسي")
            )
            result.success(true)
        } catch (error: Exception) {
